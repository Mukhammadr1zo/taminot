...
interface Row {
  gu12: string; year: number; month: number; plan: string; shipper: string
...
export default function DataTable({ dataset }: { dataset: Dataset }) {
  ...
  const rowData = useMemo<Row[]>(() => {
    const idx = filterIndices(dataset, filters, metric)
    return idx.map((i) => ({
      gu12: dataset.gu12[i],
      year: dataset.year[i],
  ...
  const columnDefs = useMemo<ColDef<Row>[]>(() => [
    ...
    { field: 'year', headerName: t('tbl.year'), width: 80, filter: 'agNumberColumnFilter' },
    { field: 'month', headerName: t('tbl.month'), width: 80, filter: 'agNumberColumnFilter' },
    ...
    { field: 'rju', headerName: t('tbl.rju'), width: 80, filter: 'agNumberColumnFilter' },
  ...
  const defaultColDef = useMemo<ColDef>(() => ({
    sortable: true, filter: true, resizable: true, floatingFilter: false, suppressHeaderMenuButton: true,
  }), [])
  ...
  const exportCsv = () => apiRef.current?.exportDataAsCsv({ fileName: 'vagon-taminoti.csv' })
  const exportXlsx = () => {
    const rows: Row[] = []
    ...
    XLSX.writeFile(wb, 'vagon-taminoti.xlsx')
  }

  return (
    <Card
      title={t('chart.table')}
      subtitle={`${fmtInt(rowData.length)} ${t('app.records')}`}
      className="col-span-full"
      right={
        <div className="flex items-center gap-1">
          <input value={quick} onChange={(e) => onQuick(e.target.value)} placeholder={t('flt.searchIn')}
            className="w-40 rounded-md border border-[var(--border)] bg-[var(--panel-2)] px-2 py-1 text-[11px] outline-none focus:border-brand-400" />
    ...
    >
      <div className={theme === 'dark' ? 'ag-theme-quartz-dark' : 'ag-theme-quartz'} style={{ height: 560, width: '100%' }}>
        <AgGridReact<Row>
          rowData={rowData}
