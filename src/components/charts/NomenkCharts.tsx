...
export function NomenkBar({ agg, dataset }: { agg: AggResult; dataset: Dataset }) {
    ...
    return {
      tooltip: {
        ...
        formatter: (ps: any[]) => {
          ...
          return `<b>${dims[g.key as number]}</b><br/>${t('lbl.plan')}: ${fmtShort(m.plan)}<br/>${t('lbl.done')}: ${fmtShort(m.done)}<br/>${t('lbl.fulfill')}: ${fmt1(pct(m.done, m.plan))}%<br/>${t('lbl.count')}: ${g.count}`
        },
      },
      ...
      series: [
        { name: t('lbl.plan'), type: 'bar', data: plan, itemStyle: { color: PLAN_COLOR, borderRadius: 3 }, barMaxWidth: 9 },
        {
          ...
          label: { show: true, position: 'right', color: c.text, fontSize: 9, formatter: (p: any) => fmtShort(p.value) },
        },
      ],
    }
  }, [rows, c, t, metric, dims])
  ...
  return (
    ...
    >
      <EChart option={option} height={Math.min(height, mode === 'all' ? height : 420)} downloadName="yuk-guruhi" />
    </Card>
  )
}
...
export function NomenkTreemap({ agg, dataset }: { agg: AggResult; dataset: Dataset }) {
  ...
  const option = useMemo(() => {
    const data = agg.byNomenk
      .map((g) => {
        const m = gm(g, metric)
        return { name: dims[g.key as number], value: Math.round(m.done), pctv: pct(m.done, m.plan) }
      })
      .filter((d) => d.value > 0)
    ...
    return {
      tooltip: {
        ...tooltipBase(c),
        formatter: (p: any) => `<b>${p.name}</b><br/>${t('lbl.done')}: ${fmtShort(p.value)}<br/>${t('lbl.fulfill')}: ${fmt1(p.data.pctv)}%`,
      },
      series: [
        {
  ...
  return (
    <Card title={t('chart.nomenkTree')} subtitle={t('lbl.share') + ' · ' + t('lbl.fulfill')}>
      <EChart option={option} height={360} downloadName="yuk-treemap" />
    </Card>
  )
}
