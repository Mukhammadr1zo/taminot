...
function StationBar({ groups, dim, title, color, name }: { groups: Group[]; dim: string[]; title: string; color: string; name: string }) {
  const { c, t, metric } = useChartCtx()
  const option = useMemo(() => {
    const rows = [...groups].sort((a, b) => gm(b, metric).done - gm(a, metric).done).slice(0, 15).reverse()
    return {
      tooltip: { trigger: 'axis', ...tooltipBase(c), axisPointer: { type: 'shadow' }, formatter: (ps: any[]) => { const g = rows[ps[0].dataIndex]; return `<b>${shortLabel(dim[g.key as number])}</b><br/>${t('lbl.done')}: ${fmtShort(gm(g, metric).done)}<br/>${t('lbl.count')}: ${g.count}` } },
      ...
      series: [{ type: 'bar', data: rows.map((g) => Math.round(gm(g, metric).done)), itemStyle: { color, borderRadius: 3 }, barMaxWidth: 13, label: { show: true, position: 'right', color: c.text, fontSize: 9, formatter: (p: any) => fmtShort(p.value) } }],
    }
  }, [groups, dim, c, t, metric, color])
  return (
    <Card title={title} subtitle={`${t('lbl.top')} 15 · ${name}`}>
      <EChart option={option} height={340} downloadName="stansiya" />
    </Card>
  )
}
...
export function StFromBar({ agg, dataset }: { agg: AggResult; dataset: Dataset }) {
  const { t } = useChartCtx()
  return <StationBar groups={agg.byStFrom} dim={dataset.meta.dims.st_from} title={t('chart.stFrom')} color="#3478f6" name={t('lbl.done')} />
}
export function StToBar({ agg, dataset }: { agg: AggResult; dataset: Dataset }) {
  const { t } = useChartCtx()
  return <StationBar groups={agg.byStTo} dim={dataset.meta.dims.st_to} title={t('chart.stTo')} color="#22c1a4" name={t('lbl.done')} />
}
...
export function NomenkMonthHeatmap({ agg, dataset }: { agg: AggResult; dataset: Dataset }) {
  ...
  const option = useMemo(() => {
    ...
    agg.byNomenkMonth.forEach((d) => {
      ...
      if (v > maxV) maxV = v
      data.push([monIndex.get(d.month)!, nomIndex.get(d.nomenk)!, v])
    })
    return {
      tooltip: { ...tooltipBase(c), formatter: (p: any) => `${nomDim[topNom[p.value[1]]]}<br/>${months[monthsPresent[p.value[0]] - 1]}: <b>${fmtShort(p.value[2])}</b>` },
  ...
  }, [agg, c, locale, metric, nomDim])
  return (
    <Card title={t('chart.heatmap')} subtitle={metric === 'tonna' ? t('metric.tonna') : t('metric.vagon')} className="col-span-full">
      <EChart option={option} height={460} downloadName="heatmap" />
    </Card>
  )
}
