...
export default function YearCompare({ agg }: { agg: AggResult }) {
  ...
  const option = useMemo(() => {
    ...
    return {
      tooltip: { trigger: 'axis', ...tooltipBase(c), axisPointer: { type: 'shadow' }, formatter: (ps: any[]) => { const i = ps[0].dataIndex; return `<b>${cats[i]}</b><br/>${t('lbl.plan')}: ${fmtShort(plan[i])}<br/>${t('lbl.done')}: ${fmtShort(done[i])}<br/>${t('lbl.fulfill')}: ${fmt1(fulfill[i])}%` } },
      ...
      series: [
        ...
        { name: t('lbl.done'), type: 'bar', data: done, itemStyle: { color: DONE_COLOR, borderRadius: [3, 3, 0, 0] }, barMaxWidth: 40, label: { show: true, position: 'top', color: c.text, fontSize: 10, formatter: (p: any) => fmtShort(p.value) } },
      ],
    }
  }, [agg, c, t, metric])
  ...
  return (
    <Card title={t('chart.compare')} subtitle={isTonna ? t('metric.tonna') : t('metric.vagon')}>
      <EChart option={option} height={320} downloadName="2025-2026" />
    </Card>
  )
}
